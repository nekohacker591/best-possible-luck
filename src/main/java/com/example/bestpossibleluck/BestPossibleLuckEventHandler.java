...
public class BestPossibleLuckEventHandler {
    ...
    private static final String TAG_ARROW_TARGET = "bplArrowTarget";
        ...
        if (!event.world.isRemote) {
            for (net.minecraft.entity.Entity entity : event.world.loadedEntityList) {
                if (entity instanceof EntityArrow) {
                    updateGuidedArrow((EntityArrow) entity);
                }
            }
        }
    }
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (LuckHooks.isPersonalLuckEnabled(event.player) && event.player.isBurning() && event.player.ticksExisted % 10 == 0) {
    ...
    public void onEntityJoin(EntityJoinWorldEvent event) {
        ...
        if (event.getEntity() instanceof EntityArrow && LuckHooks.isEnabled(world, LuckHooks.RULE_PLAYER_PROJECTILES) && !world.isRemote) {
            ...
            if (arrow.shootingEntity instanceof net.minecraft.entity.player.EntityPlayer && luckType == LuckHooks.LuckType.LUCKY) {
                ...
                if (target != null) {
                    arrow.getEntityData().setInteger(TAG_ARROW_TARGET, target.getEntityId());
                    guideArrowTowardTarget(arrow, target, shooter.dimension == 1 ? 0.95D : 0.85D);
                }
            }
        }
        ...
        if (event.getEntity() instanceof EntityZombie && LuckHooks.isEnabled(world, LuckHooks.RULE_ZOMBIE_VARIANTS)) {
            ...
            if (luckType == LuckHooks.LuckType.LUCKY && zombie.isChild()) {
                zombie.setChild(false);
            }
        }

        if (event.getEntity() instanceof EntitySlime && LuckHooks.isEnabled(world, LuckHooks.RULE_SLIME_SIZE) && luckType == LuckHooks.LuckType.LUCKY) {
            EntitySlime slime = (EntitySlime) event.getEntity();
            LuckHooks.forceSlimeSize(slime, 1);
        }
    }
    ...
    public void onEntityInteract(net.minecraftforge.event.entity.player.PlayerInteractEvent.EntityInteract event) {
        ...
        for (MerchantRecipe recipe : recipes) {
            ...
            if (!firstBuy.isEmpty()) {
                firstBuy.setCount(luckType == LuckHooks.LuckType.LUCKY ? Math.max(1, firstBuy.getCount() - 1) : firstBuy.getCount());
            }
            ItemStack secondBuy = recipe.getSecondItemToBuy();
            if (!secondBuy.isEmpty()) {
                secondBuy.setCount(luckType == LuckHooks.LuckType.LUCKY ? Math.max(1, secondBuy.getCount() - 1) : secondBuy.getCount());
            }
        }
    }
    ...
    private net.minecraft.entity.Entity findCrosshairTarget(net.minecraft.entity.player.EntityPlayer player, double range) {
        ...
        return findLookTarget(player, range, 0.85D);
    }
    ...
    private net.minecraft.entity.Entity findLookTarget(net.minecraft.entity.player.EntityPlayer player, double range, double threshold) {
        ...
        for (net.minecraft.entity.Entity entity : player.world.getEntitiesWithinAABBExcludingEntity(player, player.getEntityBoundingBox().grow(range))) {
            ...
            if (dot > bestDot + 0.005D || (dot > threshold && Math.abs(dot - bestDot) <= 0.005D && distanceSq < bestDistance)) {
                best = entity;
                bestDot = dot;
    ...
    private void updateGuidedArrow(EntityArrow arrow) {
        if (!arrow.getEntityData().hasKey(TAG_ARROW_TARGET) || arrow.isDead || arrow.inGround) {
            return;
        }
        net.minecraft.entity.Entity target = arrow.world.getEntityByID(arrow.getEntityData().getInteger(TAG_ARROW_TARGET));
        if (target == null || target.isDead) {
            arrow.getEntityData().removeTag(TAG_ARROW_TARGET);
            return;
        }
        double strength = target instanceof EntityEnderCrystal ? 0.95D : 0.85D;
        guideArrowTowardTarget(arrow, target, strength);
    }

    private void guideArrowTowardTarget(EntityArrow arrow, net.minecraft.entity.Entity target, double strength) {
        Vec3d aimPoint = getAimPoint(target);
        double leadScale = target instanceof EntityLivingBase ? 0.45D : 0.0D;
        double targetX = aimPoint.x + target.motionX * leadScale;
        double targetZ = aimPoint.z + target.motionZ * leadScale;
        Vec3d desired = new Vec3d(targetX - arrow.posX, aimPoint.y - arrow.posY, targetZ - arrow.posZ);
        if (desired.lengthSquared() < 0.0001D) {
            return;
        }
        Vec3d current = new Vec3d(arrow.motionX, arrow.motionY, arrow.motionZ);
        double speed = Math.max(2.5D, Math.sqrt(current.lengthSquared()));
        Vec3d currentDir = current.lengthSquared() < 0.0001D ? desired.normalize() : current.normalize();
    ...
    private void spawnExtraOre(World world, int chunkX, int chunkZ, net.minecraft.block.Block ore, int veins, int cluster, int minY, int rangeY) {
        ...
        for (int vein = 0; vein < veins; vein++) {
            int x = chunkX * 16 + 2 + world.rand.nextInt(12);
            int y = minY + world.rand.nextInt(Math.max(1, rangeY));
            int z = chunkZ * 16 + 2 + world.rand.nextInt(12);
            for (int i = 0; i < cluster; i++) {
                BlockPos pos = new BlockPos(x + world.rand.nextInt(3) - 1, y + world.rand.nextInt(3) - 1, z + world.rand.nextInt(3) - 1);
                if (pos.getX() < minChunkX || pos.getX() > maxChunkX || pos.getZ() < minChunkZ || pos.getZ() > maxChunkZ) {
                    continue;
                }
