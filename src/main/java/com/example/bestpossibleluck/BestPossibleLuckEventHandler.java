...
public class BestPossibleLuckEventHandler {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (LuckHooks.isPersonalLuckEnabled(event.player) && LuckHooks.isEnabled(world, LuckHooks.RULE_PARKOUR_ASSIST)) {
            applyParkourAssist(event.player);
        }
    ...
    private void applyParkourAssist(net.minecraft.entity.player.EntityPlayer player) {
        if (player.onGround || player.capabilities.isFlying || player.motionY > 0.08D) {
            return;
        }
        ...
        if (horizontalSpeedSq < 0.02D) {
            motion = new Vec3d(player.getLookVec().x, 0.0D, player.getLookVec().z);
            horizontalSpeedSq = motion.lengthSquared();
            if (horizontalSpeedSq < 0.0001D) {
                return;
            }
        }
        ...
        player.motionX += delta.x * 0.045D;
        player.motionZ += delta.z * 0.045D;
        if (horizontalDistance > 1.2D && player.motionY < -0.18D) {
    ...
    private void applyFishingHookLuck(EntityFishHook hook, EntityLivingBase target, LuckHooks.LuckType luckType) {
        if (!(hook.angler instanceof net.minecraft.entity.player.EntityPlayer)) {
            return;
        }
        net.minecraft.entity.player.EntityPlayer angler = (net.minecraft.entity.player.EntityPlayer) hook.angler;
        ...
        if (luckType == LuckHooks.LuckType.LUCKY) {
            ...
            target.motionX += direction.x * 0.35D;
            target.motionZ += direction.z * 0.35D;
