...
public class BestPossibleLuckEventHandler {
    ...
    public void onLivingUpdate(net.minecraftforge.event.entity.living.LivingEvent.LivingUpdateEvent event) {
        ...
        if (luckType == LuckHooks.LuckType.LUCKY && LuckHooks.isEnabled(world, LuckHooks.RULE_ANIMAL_BEHAVIOR) && (entity instanceof EntityAnimal || entity instanceof EntityBat || entity instanceof EntityRabbit)) {
            net.minecraft.entity.player.EntityPlayer player = world.getClosestPlayerToEntity(entity, 12.0D);
            if (player != null) {
                Vec3d target = player.getPositionVector().add(player.getLookVec());
                entity.motionX += (target.x - entity.posX) * 0.02D;
                entity.motionZ += (target.z - entity.posZ) * 0.02D;
            }
        }
        ...
        if (luckType == LuckHooks.LuckType.LUCKY && LuckHooks.isEnabled(world, LuckHooks.RULE_HOSTILE_PATHING) && entity instanceof net.minecraft.entity.monster.IMob && entity instanceof net.minecraft.entity.EntityLiving) {
            net.minecraft.entity.player.EntityPlayer player = world.getClosestPlayerToEntity(entity, 20.0D);
            if (player != null) {
                double offset = entity.ticksExisted % 40 < 20 ? 1.5D : -1.5D;
                ((net.minecraft.entity.EntityLiving) entity).getNavigator().tryMoveToXYZ(player.posX + offset, player.posY, player.posZ - offset, 1.0D);
            }
        }

        if (entity instanceof EntityDragon && luckType == LuckHooks.LuckType.LUCKY && LuckHooks.isEnabled(world, LuckHooks.RULE_DRAGON_PERCH) && entity.ticksExisted % 200 == 0) {
    ...
    public void onEntityJoin(EntityJoinWorldEvent event) {
        ...
        if (event.getEntity() instanceof EntityArrow && LuckHooks.isEnabled(world, LuckHooks.RULE_PLAYER_PROJECTILES) && !world.isRemote) {
            ...
            if (arrow.shootingEntity instanceof net.minecraft.entity.player.EntityPlayer && luckType == LuckHooks.LuckType.LUCKY) {
                net.minecraft.entity.Entity target = findLookTarget((net.minecraft.entity.player.EntityPlayer) arrow.shootingEntity, 64.0D);
                double speed = Math.sqrt(arrow.motionX * arrow.motionX + arrow.motionY * arrow.motionY + arrow.motionZ * arrow.motionZ);
                Vec3d aim = target == null ? arrow.shootingEntity.getLookVec() : target.getEntityBoundingBox().getCenter().subtract(arrow.getPositionVector()).normalize();
        ...
        if (event.getEntity() instanceof EntityFireball && LuckHooks.isEnabled(world, LuckHooks.RULE_FIREBALL_AIM) && !world.isRemote && luckType == LuckHooks.LuckType.LUCKY) {
            ...
            if (fireball.shootingEntity instanceof net.minecraft.entity.monster.IMob) {
                net.minecraft.entity.player.EntityPlayer player = world.getClosestPlayerToEntity(fireball, 64.0D);
                if (player != null) {
                    Vec3d missTarget = new Vec3d(player.posX + 5.0D, player.posY, player.posZ + 5.0D).subtract(fireball.getPositionVector()).normalize();
    ...
    public void onPlayerAttack(net.minecraftforge.event.entity.player.AttackEntityEvent event) {
        if (!LuckHooks.isEnabled(event.getEntityPlayer().world, LuckHooks.RULE_PLAYER_MELEE) || LuckHooks.LuckType.fromWorld(event.getEntityPlayer().world) != LuckHooks.LuckType.LUCKY) {
            return;
        }
        if (event.getTarget() instanceof EntityFireball) {
            ...
            if (fireball.shootingEntity != null) {
                Vec3d reflect = fireball.shootingEntity.getEntityBoundingBox().getCenter().subtract(fireball.getPositionVector()).normalize();
                fireball.motionX = reflect.x * 1.5D;
                fireball.motionY = reflect.y * 1.5D;
                fireball.motionZ = reflect.z * 1.5D;
            }
        }
        AxisAlignedBB expanded = event.getTarget().getEntityBoundingBox().grow(1.0D);
        event.getTarget().setEntityBoundingBox(expanded);
    }
    ...
    private net.minecraft.entity.Entity findLookTarget(net.minecraft.entity.player.EntityPlayer player, double range) {
        ...
        for (net.minecraft.entity.Entity entity : player.world.getEntitiesWithinAABBExcludingEntity(player, player.getEntityBoundingBox().grow(range))) {
            ...
            Vec3d toEntity = entity.getEntityBoundingBox().getCenter().subtract(start);
            double distanceSq = toEntity.lengthSquared();
            if (distanceSq > bestDistance) {
                continue;
            }
            Vec3d direction = toEntity.normalize();
            if (direction.dotProduct(look) > 0.98D) {
                best = entity;
